...
class PCAFigures():
    ...
    def __init__(self, hawkPCA):

        self.results_table = hawkPCA.results_table
        self.scores = hawkPCA.scores
        self.principal_components = hawkPCA.principal_components

    def get_score_names(PC_number):

        # PC number given using 0 index, names are 1 indexed
        PC_number = PC_number + 1

        return 'PC' + str(PC_number)
