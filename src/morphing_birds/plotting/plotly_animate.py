...
def animate_mode(
    ...
    # --- Slider labels ---
    slider_scores = (
        sign_fn(score_frames)[:, pc_number - 1]
        if sign_fn
        else score_frames[:, pc_number - 1]
    )
...
def animate_plotly(
    ...
    for frame in range(num_frames):
        ...
        fig = plot_keypoints_plotly(
            fig,
            animal3d_instance,
            colour="lightblue" if colour is None else colour,
            alpha=1,
        )
...
def animate_plotly_compare(
    ...
    for frame in range(num_frames):
        fig = go.Figure()
        for idx, (animal, keypoints) in enumerate(
            zip(animal3d_instances, formatted_list, strict=True)
        ):
            transform = (
                animal.transform_display_only
                if display_only_transform
    ...
    for frame_idx in range(num_frames):
        ...
        if leg_number is not None and any(
            ...
            fig = plot_sections_plotly(
                fig, animal3d_instance, colour=colour, alpha=alpha, section_name=sec
            )
            ...
            fig = plot_sections_plotly(
                fig, animal3d_instance, colour=colour, alpha=alpha
            )
