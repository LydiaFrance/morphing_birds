...
class TestNiceTickStep:
    """calculate_nice_tick_step should pick from {1, 2, 2.5, 5} x 10^n."""
...
class TestNiceTicksNoCollision:
    ...
    def test_hawk_ticks_inset_from_edges(self, hawk):
        ...
        margin = step * 0.1
        for t in ticks:
            assert t > x_min + margin or t == pytest.approx(
                x_min
            ), f"tick {t} too close to lower bound {x_min}"
            assert t < x_max - margin or t == pytest.approx(
                x_max
